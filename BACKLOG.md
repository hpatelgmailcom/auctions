...
### Dashboard, Reporting UI, and API

The user-facing layer that surfaces everything produced by the scraper, enrichment tools, and agents. Without this, the entire pipeline is only accessible via the command line and raw JSON files. All other backlog items — due diligence scores, compliance estimates, demographics, crime grades, retail rents, auction deadlines — feed into this dashboard as the single place to evaluate, filter, and act on auction opportunities.

**Prerequisite:** the *Database storage* item (Low Priority) should be promoted and completed first; the dashboard reads from a database, not flat JSON files. The database becomes the shared state that the scraper writes to and the dashboard reads from.

#### Views and Screens

**1. Pipeline Board**

Kanban-style view tracking each listing through its lifecycle:

```
Scouted → Enriching → Enriched → Under Review → BID / NO BID / CONDITIONAL → Auction Day → Closed
```

Each card shows: property address, starting bid, auction countdown timer, disposition score (once due diligence runs), and a color-coded BID recommendation badge. Cards approaching auction within 72 hours are highlighted. Cards with compliance deficiencies show a warning icon.

**2. Screening Table**

Sortable, filterable table across all listings. Columns:

| Column | Source |
|--------|--------|
| Address / city / state | `listing.address` |
| Starting bid | `auction.starting_bid_usd` |
| Max bid | `due_diligence.max_bid_usd` |
| Bid headroom | max bid − starting bid |
| Auction deadline | `auction.bidding_starts` |
| Property type / SF | `property.property_types`, `property.square_footage` |
| Disposition score | `due_diligence.disposition_score` |
| Crime grade | `market_research.crime.overall_grade` |
| Population (city) | `market_research.demographics.population` |
| Avg retail rent $/SF/yr | `market_research.retail_market.retail_asking_rent.avg_per_sf_yr` |
| Compliance status | `compliance_review.overall_status` |
| Est. repair cost | `compliance_review.estimated_immediate_repair_cost.high_usd` |
| Recommendation | `due_diligence.recommendation` |
| Enrichment status | which modules have run (icons) |

Filters: price range, state, property type, crime grade, disposition score threshold, BID/NO BID, opportunity zone, days to auction.

**3. Property Detail Page**

Full single-property view with tabbed sections:

- **Overview** — hero stats (starting bid, max bid, disposition score, recommendation), photo, address, map, auction countdown
- **Auction Terms** — bid increment, deposits, marketing fee, closing period, non-contingent flag
- **Property** — APN, SF, acreage, zoning, year built, tenancy, opportunity zone
- **Market Intelligence** — demographics card, crime grade card, retail rent comps table; all sourced from `market_research`
- **Due Diligence** — full financial model (NOI, DSCR, loan sizing, all-in cost), disposition score breakdown radar chart, risk flags list
- **Compliance** — domain-by-domain status table with code citations and low/high cost estimates; total estimated repair + licensing cost
- **New Business Activity** — business formation trend chart for the property's city/zip (once that backlog item is built)
- **Documents** — links to OM, due diligence vault, purchase contract, title report, inspection, Phase I/II (sourced from Crexi vault)
- **Audit Log** — timestamped record of every pipeline event (scraped, enriched, due diligence run, compliance checked)

**4. Market Intelligence Map**

Geographic view of all scouted listings plotted on a map. Marker color = BID recommendation (green / yellow / red). Click a marker to open the property detail panel. Layer toggles for: crime grade heatmap, population density, retail vacancy, opportunity zones, flood risk (once that item is built).

**5. Alerts & Watchlist Center**

- New listings scraped under the configured price threshold (pushed in real time when the scheduler runs)
- Auction deadlines in the next 7 / 3 / 1 days
- Listings where due diligence has not run and auction is within 5 days
- Compliance deficiencies above a configurable cost threshold
- Listings that crossed from NO BID to BID after a re-run of the due diligence agent (price reduction or new documents)

**6. Analytics & Reporting**

Aggregate views across all listings ever scouted:

- **Funnel report** — how many listings scraped → enriched → due diligence run → BID recommendations → auctions won
- **Market snapshot** — average starting bid, average disposition score, average crime grade, average retail rent by state or metro
- **Pipeline velocity** — time from scrape to enriched, enriched to BID decision, for process improvement
- **Opportunity map** — states/metros with highest ratio of BID recommendations to scouted listings (where the model finds the most value)
- **Exportable** — any table or report exportable to CSV; ties directly into the *Export to CSV* Low Priority item

#### REST API

A lightweight REST API that decouples the pipeline scripts from the dashboard and enables future integrations (mobile app, Slack bot, external underwriting tools).

```
Listings
  GET    /api/listings                    paginated list with filters
  GET    /api/listings/:id                full listing record
  GET    /api/listings/:id/summary        card-level fields only (for pipeline board)

Pipeline actions
  POST   /api/listings/:id/enrich         trigger enrichment for one listing
  POST   /api/listings/:id/due-diligence  trigger due diligence agent
  POST   /api/listings/:id/compliance     trigger compliance review agent
  POST   /api/scrape                      trigger a scraper run (with params)

Market research
  GET    /api/listings/:id/demographics   return market_research.demographics
  GET    /api/listings/:id/crime          return market_research.crime
  GET    /api/listings/:id/retail-market  return market_research.retail_market

Due diligence
  GET    /api/listings/:id/due-diligence  return due_diligence block
  GET    /api/listings/:id/compliance     return compliance_review block

Analytics
  GET    /api/analytics/funnel            scrape → bid conversion counts
  GET    /api/analytics/market-snapshot   aggregate stats by state/metro
  GET    /api/analytics/pipeline-velocity avg time between pipeline stages

Alerts
  GET    /api/alerts                      active alerts (deadlines, new listings)
  POST   /api/alerts/settings             configure thresholds (price, score, days)

Export
  GET    /api/export/listings.csv         all listings as flat CSV
  GET    /api/export/listings/:id.pdf     single-property PDF report
```
...
|-------|--------|-----------|
| Database | **SQLite** (dev) → **Postgres** (prod) | Already in Low Priority backlog; JSON columns store the full enrichment blobs, indexed columns for filters |
| API server | **Node.js + Fastify** | Same runtime as the scraper; minimal new dependencies |
| UI framework | **React + Vite** | Fast dev server, good ecosystem for data-heavy dashboards |
| Tables | **TanStack Table** | Headless, handles large datasets with virtual scrolling |
| Charts | **Recharts** | Lightweight, composable, works with React |
| Map | **Mapbox GL JS** or **Leaflet** | Property markers, crime heatmap layer, opportunity zone polygons |
| PDF export | **Puppeteer** (already a dependency) | Render the property detail page to PDF server-side |
| Auth | **Clerk** or simple JWT | Single-user initially; multi-user if team grows |
...
│   │   └── export.js
│   ├── db/
│   │   ├── schema.sql     # Listings, enrichment, alerts, audit_log tables
│   │   ├── migrate.js     # Run on startup to apply schema
│   │   └── queries.js     # Parameterised queries (no ORM)
│   └── workers/
│       ├── scheduler.js   # Cron: nightly scrape + enrich run
│       └── alerter.js     # Checks deadlines, fires alert records
└── dashboard/
    ├── src/
    │   ├── pages/
    │   │   ├── Pipeline.jsx
